...
bot.on("message", async message => {
    ...
    if(command.startsWith(prefix)){
        if(cmd) cmd.run(bot, message, args);
            console.log(`Oppai BOT: ${message.author.username}#${message.author.discriminator} used command ${command} on ${message.guild.name}:${message.guild.id} in ${message.channel.name}:${message.channel}`);
    }
});
